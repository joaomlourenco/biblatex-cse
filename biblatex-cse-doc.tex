...
\date{Version \bltsfileversion\ (\bltsfiledae)}
...
\begin{itemize}
  \item \textbf{v2.0.4} (2026-05-16) — Improve manual completeness:
    \begin{itemize}
      \item Recommended \texttt{\textbackslash ExecuteBibliographyOptions} block now shown (was commented out).
      \item \texttt{\textbackslash textcite} and multi-cite \texttt{\textbackslash parencite} examples added to §4.
      \item \texttt{@online}: documented \texttt{[date unknown]} fallback.
      \item \texttt{@newspaper}: new subsection added to §5 and live example in §8.
      \item \texttt{@manual}: noted that it falls through to the \texttt{@misc} driver.
      \item New §6 documents filtering thesis entries by degree using \texttt{keywords} and \texttt{\textbackslash defbibfilter}.
      \item §8 preamble note explains that \texttt{\textbackslash nauthors\{\}} is a local helper, not part of the style.
    \end{itemize}
  \item \textbf{v2.0.3} (2026-05-16) — Fix discrepancies between documentation and implementation:
    \begin{itemize}
      \item \textit{"et al."} citation examples now include the correct trailing period.
      \item Journal title in article output example shown in Roman (not italic), matching implementation.
      \item Thesis example output updated to show \texttt{[master's thesis]} for \texttt{@MastersThesis} entries.
      \item \texttt{@incollection} format now shows \texttt{Edition} between book title and publisher.
      \item \texttt{@thesis} section now documents both \texttt{[dissertation]} and \texttt{[master's thesis]} labels.
      \item \texttt{@online} and \texttt{@misc} documented separately; \texttt{@online} now shows full date format.
      \item \texttt{@patent} format now shows trailing \texttt{DOI/URL}.
      \item Added \texttt{@report / @techreport} entry type to the supported types section.
      \item Options table \texttt{urldate=iso} default is now also set in \texttt{biblatex-cse.bbx}.
      \item Fixed Online References \texttt{\textbackslash printbibliography} filter: \texttt{type=online} (was \texttt{type=manual}).
      \item Fixed section title typo: ``citations and bibliography'' (was ``abd'').
    \end{itemize}
  \item \textbf{v2.0.2} (2026-05-16) — Fix discrepancies vs tf\_cse.pdf:
    \begin{itemize}
      \item \textit{"et al."} now carries a trailing abbreviation period in both citations and bibliography.
      \item Thesis entries now emit \texttt{[master's thesis]} vs \texttt{[dissertation]} based on the \texttt{type} field or \texttt{mastersthesis} keyword.
      \item \texttt{[date unknown]} now renders correctly for online entries with no year.
      \item Patent entries now include a trailing URL/DOI.
      \item Newspaper, online, and patent entries now use full \texttt{yyyy Mon dd} dates where month/day are available.
      \item \texttt{@incollection} entries now include the \texttt{edition} field after the book title.
      \item Editor label is now pluralised (\textit{editors}) when there are multiple editors in book and chapter entries.
      \item New \texttt{@report}/\texttt{@techreport} driver: \emph{Author. Year. Title. Institution. Report No.:~number. URL}
    \end{itemize}
  \item  \textbf{v2.0.1} (2026-05-15) — Fix event date formatting to correctly display year, month, and day.
